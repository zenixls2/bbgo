	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				var spread = bestAsk.Price - bestBid.Price
				log.Infof("spread=%f ask=%f bid=%f", spread.Float64(), bestAsk.Price.Float64(), bestBid.Price.Float64())
				// var spreadPercentage = spread.Float64() / bestBid.Price.Float64()

				// if the spread is less than 10 ticks (10 pips), skip
				if spread.Float64() < 10*s.tradingMarket.TickSize {
					log.Warn("spread too small: spread=%f bid=%f ask=%f", spread.Float64(), bestBid.Price.Float64(), bestAsk.Price.Float64())
					continue
				}
				...
				var quoteAmount = midPrice.Float64() * quantity
				if quoteAmount <= s.tradingMarket.MinNotional {
					quantity = math.Max(
						s.tradingMarket.MinQuantity,
						s.tradingMarket.MinNotional*1.01/midPrice.Float64())
				}
